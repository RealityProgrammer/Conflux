...
public class CommunityService(
    ...
    public async Task<bool> BanMemberFromUserIdAsync(Guid communityId, Guid userId, TimeSpan banDuration) {
        ...
        return await BanMemberFromUserIdAsync(dbContext, communityId, userId, banDuration);
    }

    public async Task<bool> BanMemberFromUserIdAsync(ApplicationDbContext dbContext, Guid communityId, Guid userId, TimeSpan banDuration) {
        if (banDuration < TimeSpan.Zero) {
            return false;
        }
        ...
        int affected = await dbContext.CommunityMembers
            .IncludeBanned()
            .Where(r => r.CommunityId == communityId && r.UserId == userId)
            .ExecuteUpdateAsync(builder => {
                builder.SetProperty(
                    m => m.UnbanAt, 
                    m => m.UnbanAt == null ? DateTime.UtcNow + banDuration : m.UnbanAt.Value + banDuration
