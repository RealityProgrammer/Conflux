...
public interface IConversationService {
    ...
    Task<SendStatus> SendMessageAsync(Guid conversationId, Guid senderId, string? body, Guid? replyMessageId, IReadOnlyCollection<UploadingAttachment> attachments, CancellationToken cancellationToken = default);
    Task<bool> DeleteMessageAsync(Guid messageId, Guid senderId);
    Task<bool> EditMessageAsync(Guid messageId, string body);
    Task<bool> EditMessageAsync(Guid messageId, Guid senderId, string? body);
    ...
    public record RenderingMessageDTO(Guid MessageId, Guid SenderId, string SenderDisplayName, string? SenderAvatar, string? Body, DateTime CreatedAt, bool IsEdited, Guid? ReplyMessageId, List<MessageAttachment> Attachments);
